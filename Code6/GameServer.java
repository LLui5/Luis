...
class Listgamenumber{
	...
	class MyTimer implements Runnable{
		...
		public void run(){
			while(true){
				for (int i = 0; i < playerCount; i++)
					players[i].send("k"+0+":"+players[0].x+":"+players[0].y+"s"+seconds+"p"+puntos);
				seconds=seconds-1;
				interval=interval+1;
				...
				if(interval>21)
					for(int j=0;j<4;j++)
					{
					listn[j].value=(seconds+15*j)%9;
					listn[j].x=interval+seconds-(value*j);
					listn[j].y=seconds/2+interval+(value*j);	
					}
